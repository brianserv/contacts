...
int32_t CGetUinHandler::GetUin(ICtlHead *pCtlHead, IMsgHead *pMsgHead, IMsgBody *pMsgBody, uint8_t *pBuf, int32_t nBufSize)
{
	...
	AccountInfo *pAccountInfo = (AccountInfo *)g_Frame.GetConfig(ACCOUNT_INFO);
	...
	string strKey = ACCOUNTID + pGetUinReq->m_strAccountID;
	pAccountIDChannel->Get(pSession, (char *)strKey.c_str());
...
int32_t CGetUinHandler::OnSessionGetAccountName(int32_t nResult, void *pReply, void *pSession)
{
	...
	else
	{
		...
		AccountInfo *pAccountInfoConfig = (AccountInfo *)g_Frame.GetConfig(ACCOUNT_INFO);
		CRedisChannel *pAccountInfoChannl = pRedisBank->GetRedisChannel(ACCOUNT_INFO);
		pAccountInfoChannl->HMGet(pRedisSession, (char *)pUserSession->m_strAccountName.c_str(), "%s", pAccountInfoConfig->uin);
	}
...
int32_t CGetUinHandler::OnSessionGetUserUin(int32_t nResult, void *pReply, void *pSession)
{
	...
	else
	{
		...
		UserBaseInfo *pUserBaseInfoConfig = (UserBaseInfo *)g_Frame.GetConfig(USER_BASEINFO);
		CRedisChannel *pUserBaseInfoChannl = pRedisBank->GetRedisChannel(USER_BASEINFO);
		pUserBaseInfoChannl->HMGet(pRedisSession, itoa(pUserSession->m_nUin), "%s", pUserBaseInfoConfig->version);
	}
